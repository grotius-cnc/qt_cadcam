#include "preview_displacement.h"
...
preview_displacement::preview_displacement(int i, double dist_x, double dist_y, double dist_z)
{
    //draw objects from cad array..
    if(int_cad_array[i][0] == 10){ // id text..
        (display_user_text(i,dist_x,dist_y));
    }
    ...
    if(int_cad_array[i][0] == 2){ // id = circle
        ...
        for (int ii = 0; ii < segments; ii++)   {
            ...
            glVertex3d(x + double_cad_array[i][6]/*x center*/+dist_x, y + double_cad_array[i][7]/*y center*/+dist_y, double_cad_array[i][8]/*z center*/+dist_z);        //output vertex
        }
        glEnd();
        //glDisable(GL_LINE_STIPPLE);
    ...
    if(int_cad_array[i][0] == 3){ // id = arc
        ...
        for(int ii = 0; ii < segments; ii++)
        {
            glVertex2f(x + double_cad_array[i][6]+dist_x, y + double_cad_array[i][7]+dist_y);
    ...
    if(int_cad_array[i][0] == 4){ // id = ellipse
        ...
        for(int i = 0; i<360; i++){
            ...
            glVertex2d(rotate_x0+dist_x, rotate_y0+dist_y);
        }
        glEnd();
        //glDisable(GL_LINE_STIPPLE);
    ...
    if(int_cad_array[i][0] == 5){ // id = qubic bezier curve
        ...
        for(double t = 0; t < 1; t+=0.001){ //choose a different resolution for t ..
            ...
            glVertex3d(Px+dist_x, Py+dist_y, Pz+dist_z);  //draw linestrip..
        }
        glEnd();
        //glDisable(GL_LINE_STIPPLE);
    ...
    if(int_cad_array[i][0] == 7){ // id = point
